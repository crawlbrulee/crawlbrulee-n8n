...
const sign = (t: number, s = secret) =>
	`t=${t},v1=${createHmac('sha256', s).update(`${t}.${payload}`).digest('hex')}`;
...
describe('verifySignature', () => {
	it('verifies the primary header', async () => {
		expect(
			await verifySignature({
				payload,
				headers: { 'x-cwbl-signature': sign(now) },
				secret,
				nowSeconds: now,
			}),
		).toEqual({ verified: true, signedWith: 'primary' });
	});
	...
	it('falls back to the rotated header', async () => {
		const r = await verifySignature({
			payload,
			headers: { 'x-cwbl-signature': sign(now, 'old'), 'x-cwbl-signature-rotated': sign(now) },
			secret,
			nowSeconds: now,
		});
	...
	it('reports each failure reason', async () => {
		expect(await verifySignature({ payload, headers: {}, secret, nowSeconds: now })).toEqual({
			verified: false,
		...
		expect(
			await verifySignature({
				payload,
				headers: { 'x-cwbl-signature': 'garbage' },
				secret,
				nowSeconds: now,
			}),
		).toEqual({ verified: false, reason: 'malformed_signature' });
		expect(
			await verifySignature({
				payload,
				headers: { 'x-cwbl-signature': sign(now - 301) },
				secret,
				nowSeconds: now,
			}),
		).toEqual({ verified: false, reason: 'timestamp_out_of_tolerance' });
		expect(
			await verifySignature({
				payload,
				headers: { 'x-cwbl-signature': sign(now, 'wrong') },
				secret,
				nowSeconds: now,
			}),
	...
	it('ignores tolerance when it is 0', async () => {
		expect(
			await verifySignature({
				payload,
				headers: { 'x-cwbl-signature': sign(now - 99999) },
				secret,
				nowSeconds: now,
				toleranceSeconds: 0,
			}),
	...
	it('rejects an unsafe-integer timestamp as malformed, like the sdk', async () => {
		...
		expect(
			await verifySignature({
				payload,
				headers: { 'x-cwbl-signature': sign(t) },
				secret,
				nowSeconds: now,
				toleranceSeconds: 0,
			}),
	...
	it('falls back to the rotated header when the primary is stale', async () => {
		const r = await verifySignature({
			payload,
			headers: { 'x-cwbl-signature': sign(now - 301), 'x-cwbl-signature-rotated': sign(now) },
			secret,
			nowSeconds: now,
		});
		expect(r).toEqual({ verified: true, signedWith: 'rotated' });
	...
	it('reports malformed when both headers are malformed', async () => {
		const r = await verifySignature({
			payload,
			headers: { 'x-cwbl-signature': 'garbage', 'x-cwbl-signature-rotated': 'garbage' },
			secret,
			nowSeconds: now,
		});
	...
	it('ranks a mismatch above a malformed header', async () => {
		const r = await verifySignature({
			payload,
			headers: { 'x-cwbl-signature': 'garbage', 'x-cwbl-signature-rotated': sign(now, 'wrong') },
			secret,
			nowSeconds: now,
		});
	...
	it('reports a timestamp a year in the future as out of tolerance', async () => {
		const r = await verifySignature({
			payload,
			headers: { 'x-cwbl-signature': sign(now + 31_536_000) },
			secret,
			nowSeconds: now,
		});
