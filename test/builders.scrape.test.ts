...
import { buildAsyncScrapeBody, buildScrapeBody, type ScrapeParams } from '../nodes/Crawlbrulee/builders/scrape';
...
describe('buildScrapeBody', () => {
	it('sends the url and all six extract flags explicitly', () => {
		expect(buildScrapeBody(node, 0, base())).toEqual({
			url: 'https://example.com',
			extract: { markdown: false, cleaned_html: true, raw_html: false, links: false, images: false, metadata: true },
		});
	});
	...
	it('maps every option', () => {
		const body = buildScrapeBody(node, 0, base({
			options: {
				proxy: 'advanced', requireJs: true, removeAdsAndPopups: false,
				excludeSelectors: ['.ad', ''], cacheMaxAge: '3600', locale: 'de-DE', country: 'de',
			},
		}));
	...
	it('passes an ISO cache cutoff as a string', () => {
		expect(buildScrapeBody(node, 0, base({ options: { cacheMaxAge: '2026-09-01T00:00:00Z' } })).cache).toEqual({
			max_age: '2026-09-01T00:00:00Z',
		});
	});

	it('does not send require_js: false', () => {
		expect(buildScrapeBody(node, 0, base({ options: { requireJs: false } }))).not.toHaveProperty('require_js');
	});
	...
	it('builds a full screenshot block', () => {
		const body = buildScrapeBody(node, 0, base({
			...
			screenshotOptions: {
				width: 1280, height: 800, deviceScaleFactor: 2, deviceMode: 'mobile',
				actionsBefore: { action: [{ type: 'wait', value: 500 }, { type: 'scroll', value: 1000 }] },
				sliceHeight: 900,
			},
		}));
		expect(body.extract?.screenshot).toEqual({
			...
			device_mode: 'mobile',
			actions_before: [{ type: 'wait', ms: 500 }, { type: 'scroll', pixels: 1000 }],
			actions_after: [{ type: 'slice', height: 900 }],
		});
	});

	it('sends a bare screenshot type when no options are set', () => {
		expect(buildScrapeBody(node, 0, base({ screenshotType: 'viewport' })).extract?.screenshot).toEqual({ type: 'viewport' });
	});

	it('rejects width without height, and a scale factor without a viewport', () => {
		expect(() => buildScrapeBody(node, 2, base({ screenshotType: 'viewport', screenshotOptions: { width: 100 } }))).toThrow(NodeOperationError);
	...
	it('rejects more than 5 actions and a slice under 500', () => {
		...
		expect(() => buildScrapeBody(node, 0, base({ screenshotType: 'viewport', screenshotOptions: { sliceHeight: 100 } }))).toThrow(/at least 500/);
	});
});

describe('buildAsyncScrapeBody', () => {
	it('adds the webhook when a url is set, parsing json metadata', () => {
		const body = buildAsyncScrapeBody(node, 0, base({ webhookUrl: 'https://hook', webhookMetadata: '{"tenant":"acme"}' }));
	...
	it('accepts object metadata and omits the webhook without a url', () => {
		expect(buildAsyncScrapeBody(node, 0, base({ webhookUrl: 'https://hook', webhookMetadata: { a: 1 } })).webhook).toEqual({ url: 'https://hook', metadata: { a: 1 } });
	...
	it('rejects metadata without a url and invalid metadata json', () => {
		expect(() => buildAsyncScrapeBody(node, 0, base({ webhookMetadata: '{"a":1}' }))).toThrow(/Webhook URL/);
		expect(() => buildAsyncScrapeBody(node, 0, base({ webhookUrl: 'https://hook', webhookMetadata: '{oops' }))).toThrow(/Webhook Metadata/);
	});
});
